...
export function ApiFiles(files: _.Many<IApiFile>, options: Partial<{ isRequired: boolean }> = {}): MethodDecorator {
  const filesArray = _.castArray(files);
  const apiFileInterceptors = UseInterceptors(FileFieldsInterceptor(filesArray.map((v) => ({ name: v.name }))));
  // const apiFileInterceptors = filesArray.map(
